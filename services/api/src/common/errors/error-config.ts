...
export function getErrorFrameworkConfig(
  config: ConfigService,
): ErrorFrameworkConfig {
  return {
    enabled: readBoolean(config, 'ERROR_LOG_ENABLED', true),
    storage: readStorage(config.get<string>('ERROR_LOG_STORAGE') ?? 'database'),
    retentionDays: readPositiveNumber(
      config.get<string>('ERROR_LOG_RETENTION_DAYS'),
      90,
    ),
    includeStack: readBoolean(config, 'ERROR_LOG_INCLUDE_STACK', true),
    includeRequestBody: readBoolean(
      config,
      'ERROR_LOG_INCLUDE_REQUEST_BODY',
      false,
    ),
    exposeStackToSystemCustomizer: readBoolean(
      config,
      'ERROR_LOG_EXPOSE_STACK_TO_SYSTEM_CUSTOMIZER',
      true,
    ),
    downloadRole:
      config.get<string>('ERROR_LOG_DOWNLOAD_ROLE')?.trim() ||
      'System Customizer',
    traceHeader: (
      config.get<string>('ERROR_TRACE_HEADER')?.trim() || 'x-trace-id'
    ).toLowerCase(),
    supportMessage:
