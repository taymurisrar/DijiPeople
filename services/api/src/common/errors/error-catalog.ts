...
export const ERROR_CATALOG = {
  ...
  /*
   * TASK-0032. Codes thrown by MFA (ADR-0019), platform authorization
   * (ADR-0018), agreements (ADR-0020) and the platform audit trail. Without an
   * entry here the exception filter keeps the message but replaces the code
   * with a status-derived generic one, so a client could never tell, say, an
   * expired sign-in challenge from a wrong code.
   */
  ADMIN_AUTH_INVALID_CREDENTIALS: entry(
    401,
    'Invalid admin credentials',
    'The email or password is not correct, or the account cannot sign in right now.',
    'warning',
    'auth',
  ),
  AUTH_MFA_CHALLENGE_INVALID: entry(
    401,
    'Sign-in expired',
  ...
  MFA_CODE_INVALID: entry(
    400,
    'Invalid code',
    'That code does not match. Enter the current code from your authenticator app.',
    'warning',
    'auth',
  ),
  MFA_ALREADY_ENABLED: entry(
    409,
    'Two-factor authentication already on',
    'Two-factor authentication is already turned on for this account.',
    'info',
    'auth',
  ),
  MFA_NOT_ENABLED: entry(
    400,
  ...
  MFA_SETUP_NOT_STARTED: entry(
    400,
    'Setup not started',
    'Start two-factor setup before entering a code.',
    'warning',
    'auth',
  ),
  MFA_SETUP_SUPERSEDED: entry(
    409,
    'Setup restarted',
    'Two-factor setup was restarted elsewhere. Scan the newest QR code.',
    'warning',
    'auth',
  ),
  MFA_FACTOR_REQUIRED: entry(
    400,
    'Code required',
  ...
  MFA_RESET_SELF: entry(
    400,
    'Cannot reset your own two-factor authentication',
    'Turn two-factor authentication off from your own security settings instead.',
    'warning',
    'auth',
  ),
  MFA_ENCRYPTION_UNAVAILABLE: entry(
    503,
    'Two-factor authentication unavailable',
    'Two-factor authentication cannot be set up until the server encryption key is configured.',
    'critical',
    'settings',
  ),
  CURRENT_PASSWORD_INVALID: entry(
    400,
  ...
  PLATFORM_PERMISSION_DENIED: entry(
    403,
    'Platform permission required',
    'Your platform role does not allow this action.',
    'warning',
    'rbac',
  ),
  PLATFORM_MONITORING_PERMISSION_REQUIRED: entry(
    403,
    'Monitoring permission required',
    'Your platform role does not include monitoring access.',
    'warning',
    'rbac',
  ),
  PLATFORM_ROLE_NOT_ASSIGNABLE: entry(
    400,
    'Role cannot be assigned',
    'This platform role is retired and cannot be given to a user.',
    'warning',
    'rbac',
  ),
  PLATFORM_AUDIT_TRAIL_USE_DEDICATED_ENDPOINT: entry(
    400,
    'Use the platform audit trail',
    'Platform audit events are read from the platform audit trail.',
    'info',
    'access',
  ),
  TENANT_STATUS_REQUIRES_LIFECYCLE_ACTION: entry(
    400,
    'Use a lifecycle action',
    'A tenant status is changed through its lifecycle actions, which record a reason.',
    'warning',
    'tenant',
  ),
  CONTRACT_TEMPLATE_PLACEHOLDER_OUT_OF_CONTEXT: entry(
    400,
    'Placeholder not available for this agreement type',
    'The template uses a placeholder this agreement type cannot resolve.',
    'warning',
    'validation',
  ),
  CONTRACT_PLACEHOLDER_UNRESOLVABLE_CONTEXT: entry(
    400,
    'Placeholder cannot be resolved',
    'A required placeholder refers to a record this agreement is not linked to.',
    'warning',
    'validation',
  ),
  CONTRACT_DUPLICATE_AGREEMENT: entry(
    409,
    'Agreement already exists',
    'An open agreement of this type already exists for the same counterparty.',
    'warning',
    'validation',
  ),
  NETWORK_ERROR: entry(
