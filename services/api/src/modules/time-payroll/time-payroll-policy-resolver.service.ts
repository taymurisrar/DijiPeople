...
function scoreTimePolicy(
  ...
  const countryMatches = Boolean(
    policy.countryCode &&
    input.countryCode &&
    policy.countryCode === input.countryCode.toUpperCase(),
  );

  if (levelMatches && businessUnitMatches) return 500;
  if (levelMatches && !policy.businessUnitId && !policy.countryCode) return 400;
  if (businessUnitMatches && !policy.employeeLevelId && !policy.countryCode)
    return 300;
  if (countryMatches && !policy.employeeLevelId && !policy.businessUnitId)
    return 200;
  if (!policy.employeeLevelId && !policy.businessUnitId && !policy.countryCode)
    return 100;
  return 0;
}
