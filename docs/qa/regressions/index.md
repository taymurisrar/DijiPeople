...
| **Regression test** | `services/api/src/modules/billing/flat-pricing-is-internal.spec.ts` |
| **Scenario** | Four cases over the shared `narrowestSalesModel` predicate, in the same file that already guards the resolver — so both readers of the rule are asserted together and cannot drift apart again. A `SALES_ASSISTED` price under a `SELF_SERVICE` plan is excluded; the per-seat price the visitor is meant to see is included; every price of a `CUSTOM_ONLY` plan is excluded even where the row says `SELF_SERVICE`; and every price of a `SALES_ASSISTED` plan is excluded. The last two pin the narrowing direction: the plan's model narrows the price's and never widens it. |
| **Proven to fail without the fix** | The defect proved itself in production. Before the change, `/api/public/plans` returned four active QAR prices for Starter — `MONTHLY FLAT 249`, `MONTHLY PER_SEAT 8`, `ANNUAL FLAT 2490`, `ANNUAL PER_SEAT 80` — while `/api/public/commercial-config`, over the same rows, returned only the two per-seat offers. |
| **Note** | **Writing a rule down is not the same as applying it.** `flat-pricing-is-internal.spec.ts` states the policy in prose, cites the owner, and explains why the mechanism is `salesModel` rather than a `billingModel` filter — and it guarded exactly one of the two endpoints that needed it. The second endpoint was older, read the same table, and nothing in the types or the tests connected them. Note also what the fix deliberately did *not* do: test `billingModel` in `getPublicPlans`. That would have been faster to write, correct today, and a second rule for one decision — which is the defect, not the repair. The spec's own header warns about this in advance, and following that warning is why the fix is three lines and a shared import. Two harms, and the milder one came first: internal hand-negotiated rates were readable without authentication for as long as the endpoint existed, and became *sellable* the moment prices were synced to Stripe. |
| **Fixed** | 2026-08-25, `agent/landing-qa-fixes` |
| **Active** | yes |
