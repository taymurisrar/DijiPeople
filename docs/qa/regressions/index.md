...
### REG-390 — The approvals inbox rendered a blank record and could not decide anything

| | |
|---|---|
| **Bug class** | `response-envelope-mismatch` |
| **Module** | `services/api/src/modules/approvals` |
| **Bug record** | BUG-2718 |
| **Root cause** | `GET /approvals/:id` answers `{ item: {...} }` and `GET /approvals` answers a bare list. The record page declared `apiRequestJson<ApprovalRequestItem>` and then read `approval.title`, `approval.moduleKey` and `approval.submittedByUser` off the envelope, so every field was `undefined` while the same row rendered correctly in the list. `apiRequestJson<T>` casts and does not validate, so the compiler had nothing to object to. Separately `detail()` never derived `currentStep` although `list()` did, and Approve/Reject were `disabledBusinessCommand` stubs with no controller route, no proxy POST and no adapter handler behind them. |
| **Regression test** | `services/api/src/modules/approvals/approvals.decision.spec.ts`, `apps/web/lib/runtime/modules/approval-decision-commands.spec.ts` |
| **Scenario** | Assert `detail()` derives `currentStep`, and that its decision block reports what the caller may actually do. Assert `decide()` dispatches to the module that raised the request rather than writing the approval row, refuses a caller lacking the owning module's permission, refuses a module with no delegate by name, refuses a request that is not pending, and scopes its lookup by tenant and caller reach rather than by id alone. On the client, assert the commands are not spec-level disabled, that each takes its enabled state from a per-record capability field, and that the handler posts to `/api/approvals/<id>/{approve,reject,cancel}`. |
| **Proven to fail without the fix** | Removing the `currentStep` derivation, and separately bypassing the permission evaluation, each failed exactly its own test and nothing else. Restoring the `disabledBusinessCommand` stubs failed four client assertions; removing the adapter branch failed five. |
| **Note** | Three things generalise. **A generic inbox must decide through the module that owns the record, never by moving its own row**: `ApprovalRequest` is a mirror for leave and attendance, `LeaveApprovalStep` is authoritative, and advancing the mirror would have reported APPROVED on this screen while the leave request stayed PENDING with no balance consumed. **Dispatching in-process skips the controller, so it skips the guard** — the owning module's permission is evaluated by the same function `PermissionsGuard` calls, extracted rather than reimplemented, because two copies of "who may approve a leave request" is the BUG-2015 shape. **A disabled button must say why in terms of the record**, not in terms of the runtime: the old caption named a missing adapter handler, so a caller could not tell an already-decided request from one assigned to someone else from one whose module is decided elsewhere. |
| **Fixed** | 2026-08-31, branch `agent/approvals-inbox-decisions` |
| **Active** | yes |

### REG-391 — A sink provider recorded every message as SENT, and LOG_LEVEL=info hid the evidence

| | |
|---|---|
| **Bug class** | `success-reported-for-a-discarded-side-effect` |
| **Module** | `services/api/src/modules/notifications/email` |
| **Bug record** | BUG-2741 |
| **Root cause** | Success was defined as "the provider did not throw", which is the right test for a transport and the wrong one for a sink whose entire behaviour is to accept and discard. `CONSOLE` and `DEV` both resolve to `ConsoleEmailProvider`, which returns a `providerMessageId` exactly as SMTP does, so the delivery log recorded `SENT` and the schedule recorded `COMPLETED` for a message nobody received. `EmailDeliveryStatus` had no value meaning "accepted and not delivered", so the honest answer was unavailable. A second defect hid it: Nest names the informational level `log` while every other ecosystem names it `info`, the live service carried `LOG_LEVEL=info`, `indexOf` returned -1 and it fell back to the production default silently — discarding the console provider's own record of each message it swallowed. |
| **Regression test** | `services/api/src/modules/notifications/email/email-sink-delivery-status.spec.ts`, `services/api/src/modules/notifications/email/email-delivery-capability.spec.ts`, `services/api/src/log-level-resolution.spec.ts` |
| **Scenario** | Drive `execute()` to a successful provider send and assert the delivery log status: `NOT_DELIVERED` for CONSOLE and for DEV, still `SENT` for SMTP, with `sent` staying true in every case and `delivered` following the provider. Assert the undelivered summary is logged at warn and not at log. Assert the capability resolver reports a tenant SMTP provider as able to deliver, a tenant CONSOLE sink as not, a platform relay the tenant depends on as able, a platform relay that is itself a sink as not, and no provider at all as not. Assert `LOG_LEVEL=info` yields error/warn/log, and that an unrecognised value both falls back and warns naming the accepted values. |
| **Proven to fail without the fix** | Restoring the unconditional `SENT` failed three of the four delivery-status cases and left the SMTP case green, which is correct. Removing the `info` alias and the unrecognised-value warning failed both log-level cases. Making the capability ignore sinks failed both sink cases. |
| **Note** | Three things generalise. **A pipeline that reports success is not evidence that the artifact exists** — the schedule said COMPLETED, the delivery log said SENT with a plausible message id, and the subject line had even rendered correctly; every layer was truthful about its own step and the whole was false. **Do not overload an existing status to avoid a migration**: `DRY_RUN` means the caller requested a rehearsal and `SKIPPED` means the send was suppressed before a provider was reached, and reusing either for "your workspace cannot deliver" would have made all three unreadable. **Resolve a capability through the path the real operation takes**: `resolveProviderForOrigin` slots the platform relay between the tenant's own providers and the environment fallback, so a check reading `listEnabledProviders` alone would tell every tenant relying on the platform relay that it cannot send. Also worth keeping: the log line for the undelivered case is `warn` rather than `log` on purpose, because production resolves to error and warn only — logging the discovery of silent logging at a silenced level would have repeated the bug. |
| **Fixed** | 2026-08-31, branch `agent/email-sink-visibility` |
| **Active** | yes |

### REG-392 — A business refusal was rendered as a fatal error dialog

| | |
|---|---|
| **Bug class** | `expected-outcome-shown-as-a-defect` |
| **Module** | `apps/web/lib/runtime` |
| **Bug record** | BUG-2822 |
| **Root cause** | The split between "the product refused" and "the product broke" existed, and was owned by one module. `lib/attendance/attendance-outcome.ts` had been written after the same complaint about a refused check-in, so attendance answered its own policy outcomes in place while every other module fell through to `dispatchCommandFailure` and the technical dialog. Pressing Approve on your own approval therefore produced a red ERROR ACCESS_DENIED heading, a reference id, a timestamp and a Download log button for a rule the product had just enforced correctly. A second, smaller cause in the rendering: `HttpExceptionFilter` fills `description` with a generic sentence when the thrower supplies none, so a precise headline sat above a vague one and read like boilerplate itself. |
| **Regression test** | `apps/web/lib/runtime/command-failure-classification.spec.ts` |
| **Scenario** | Assert a 400, 403, 409 or 422 carrying the API envelope classifies as `business` and surfaces the server's own message. Assert the opposite direction harder: 5xx, 401, 404, an HTML body, a multi-line stack trace, and a payload with no server-written message all stay `unexpected` and keep the technical dialog. Assert a generic `description` that only repeats the headline is dropped, and a specific one is kept. |
| **Proven to fail without the fix** | Removing the status gate — every failure becomes `business` — fails seven cases including all four 5xx, the 401 and the 404. Separately removing the envelope shape check fails three: the HTML body, the multi-line stack trace, and the payload carrying only the reader's own "Command failed" fallback. |
...
### REG-394 — Two safety gates composed into a deadlock, so no on-premise attendance integration could ever be activated

| | |
|---|---|
| **Bug class** | `mutual-precondition-deadlock` |
| **Module** | `services/api/src/modules/attendance-integrations`, `gateway/src/DijiPeople.Gateway.Host` |
| **Bug record** | BUG-2732 |
| **Root cause** | Two independently correct rules with no first move between them. The API refused to activate an integration until one of its devices reported `VERIFIED`, so a misconfigured terminal could not silently start writing attendance. The gateway refused to touch a device whose integration was not active, so a draft configuration could not dial a customer's network. The only writer of `VERIFIED` is the gateway's verification report, and the only path to that report ran through the loop the second rule guarded — so the terminal stayed `UNVERIFIED` for ever. Neither author could see it alone; it exists only in the composition. The product also shipped a readiness message instructing administrators to "run Verify device", an action that had no route and no control anywhere in the codebase. |
| **Regression test** | `gateway/tests/DijiPeople.Gateway.Tests/ActivationDeadlockTests.cs` |
| **Scenario** | With a paired gateway, an enabled device and an integration still `UNVERIFIED`, an administrator asks to verify the device. The gateway must run the verification, report it, and stop — reading no attendance and queueing nothing — after which activation succeeds. An integration that is `DISABLED` or `ERROR` must not be dialled by the same path. |
| **Proven to fail without the fix** | Proven by live rehearsal rather than by the unit test, and the distinction matters. The unit test covers `AwaitsVerification`, a function the fix introduces, so it cannot fail against the old code — it guards the DISABLED/ERROR boundary, not the deadlock. The deadlock itself was proven twice: by reading the four code paths (`attendance-integration.service.ts:372`, `:447-449`, `gateway-runtime.service.ts:135`, `GatewayWorker.cs:307`/`:397`), and by an end-to-end run on the local `xoul-ltd` tenant against the bundled K50 simulator, where a device that had sat `UNVERIFIED` since 31 August verified within one configuration refresh and the integration then activated with `blockers: []`. Before the fix that same request had been outstanding and unacknowledged for nine days. |
...
### REG-396 — A second information architecture nobody connected to the gate built for the first

| | |
|---|---|
| **Bug class** | `parallel-structure-missed-by-a-fix` |
| **Module** | `apps/web/app/(authenticated)/settings`, `services/api/src/common/constants/tenant-features.ts` |
| **Bug record** | BUG-2958 |
| **Root cause** | BUG-1952 built entitlement enforcement and applied it by API module directory: a register of gated modules, a guard, and an invariant spec that fails when a controller in a gated directory lacks the decorator. That is complete for route modules and cannot see the settings tree, which is a separate structure with its own resolver, its own 87-item registry and no directory of its own. `resolveVisibleSettingsRuntime` took permissions and roles and nothing else, so a Starter tenant was offered the whole Payroll & Finance tree. The register even names the gap in writing — `branding` is exempted there because it is "enforced where settings resolve, not by a route gate" — and nothing was enforcing where settings resolve. |
| **Regression test** | `apps/web/app/(authenticated)/settings/_lib/settings-entitlements.spec.ts` |
| **QA scenario** | QA-SETTINGS-017 |
| **Scenario** | Resolve the settings IA with the Starter capability set and every settings permission, as each of the three administrator roles. No Payroll & Finance category, no Payroll Geography group, no `timesheets` row inside a surviving Attendance & Time group, no Apps & Modules group. Resolve with every capability: all eleven categories return. Resolve with `null`: throw rather than filter. Resolve with `[]`: the core pages, not an error. Every item in the built registry has an attribution. |
| **Proven to fail without the fix** | Mutation-tested twice. Forcing `isSettingsItemEntitled` to return `true` — the shape the code had before this change — fails 8 of 21. Replacing the fail-closed throw with an empty set fails 1, which is the assertion that separates "we could not read the plan" from "the plan includes nothing". |
...
### REG-543 — Every platform sign-in outcome is audited, without the attempted password
...
| **QA scenario** | — |
| **Scenario** | Success writes `AUTH_LOGIN_SUCCEEDED` (`tenantId: 'platform'`, actor = operator, `mfaResult`, session id, forwarded IP and user agent); wrong password, locked account, unknown address (no actor, entity = the address tried) and wrong MFA code write `AUTH_LOGIN_FAILED` with the `failureReason`; every failure keeps the identical 401 body; no row contains the attempted password. |
| **Proven to fail without the fix** | Mutation: making `logPlatformAuthEvent` return early fails 4 of the spec's 5 tests. |
| **Note** | Same snapshot shape as tenant sign-in rows, so the audit screens read both alike. |
| **Fixed** | 2026-09-25, TASK-0032 WP-03 on branch `agent/pah-wp03-mfa` (see WP-03 report for the commit) |
| **Active** | yes |
...
### REG-601 — `PATCH /partners/:id` was never actually partial
...
### REG-607 — `DELETE /super-admin/customers` admitted a non-admin-tier role the runtime delete path already refused
...
### REG-608 — `DELETE /super-admin/customer-onboarding` admitted the same non-admin-tier gap
