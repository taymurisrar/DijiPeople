---
SCENARIO_ID: QA-BILLING-016
aliases: [QA-BILLING-016]
TITLE: A flat plan price bills one subscription, not one seat per head
AREA: seat-billing
MODULE: services/api/src/modules/billing
TYPE: UNIT
RISK: HIGH
AUTOMATION_STATUS: AUTOMATED
TEST_REFERENCE: services/api/src/modules/billing/billing-seat-pricing.spec.ts
RELATED_BUGS: [BUG-0901]
RELATED_REGRESSIONS: [REG-235]
LAST_RUN: 
LAST_RESULT: NOT_RUN
CREATED_AT: 2026-08-23
UPDATED_AT: 2026-08-23
---

# QA-BILLING-016 — A flat plan price bills one subscription, not one seat per head

## Preconditions

The state the system must be in. Roles, tenants, seeded data, feature flags.

## Steps

1. 

## Expected Result

What correct looks like — written **before** the scenario is run. Deciding what
"correct" means after seeing the output is not testing.

## Notes

Created 2026-08-23 at `539d99ce`.

<!-- GRAPH:BEGIN — generated by scripts/rebuild-qa.mjs; edit the frontmatter, not this block -->

## Related

- Test plan — [[PLAN-016]]
- Bugs — [[BUG-0901]]
- Regressions — REG-235 (see the regression register)

<!-- GRAPH:END -->
