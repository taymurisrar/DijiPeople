...
- WP-04 partial: config plumbing + `ClipboardWatcher` + `DlpRuleEvaluator` with
  full unit coverage are done (`cd23ac7`, 67/67 agent-desktop tests green). Still
  in WP-04: the orchestrator that composes them, `desktopCapturer` screenshot
  capture, the tray active-capture indicator, `ApiClient` send methods, and the
  `session-manager` wiring. Those align to the WP-02 API contract and are best
  built once the endpoints exist.
- **Blocking the rest:** WP-01 (schema + migration) needs a throwaway local
  `DATABASE_URL` (never the populated `dijipeople` dev DB). WP-02/03/05 depend on
  WP-01.

## History

- 2026-08-24 — created at `bb740183`.
- 2026-08-25 — ExecPlan `PLAN-022`; WP-04 detection core implemented at
  `cd23ac7`.
