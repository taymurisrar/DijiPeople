...
- 2026-08-24, later — **confirmed by the reconcile itself, running in
  production.** The entry above was derived statically, by comparing the
  catalogue against the public plans endpoint. Deploy
  `dep-da68nme1egvs739pn1ag` then ran `seed:config`, which calls the same
  `bootstrapCommercialDefaults`, and reported:

  ```
  Commercial catalogue: 0 plan(s) created, 0 reconciled, 0 retired,
    0 withdrawn from sale; 0 market(s) created; 0 price(s) …
  ```

  Every counter zero. The static prediction and the live run agree, and the
  agreement is worth more than either alone: the static method could not see the
  partial unique index that assumption **A-06** names as its own limitation, and
  a real reconcile against the real schema is exactly what closes that gap.

- 2026-08-25 — closed. All four work packages were already `DONE`; the header
  had been left `IN_PROGRESS` and the record carried no
  `NEXT_READY_WORK_PACKAGE` at all, so a resuming session had nothing to start
  from. Verified rather than assumed: `0a5586f` (WP-04) is an ancestor of
  `origin/develop`, and `origin/agent/plans-reset` is fully merged into it.
