...
COMPLETED_PACKAGES: [WP-01, WP-02, WP-03, WP-04, WP-05, WP-06, WP-08, WP-09, WP-10, WP-12]
...
| WP-07 | Payment to onboarding to provisioning automation, steps, resumability, targets | IN_PROGRESS | WP-01, WP-05 | Backend/API, Database, Integration | agent/commercial-platform-completion | 943a826 | PASS | PASS | DONE |
...
- 2026-08-19 — **WP-07 reopened by TASK-0008.** It closed `DONE` with
  `QA_STATUS = PASS` and half its scope unbuilt. Two defects, one root:
  [[BUG-0077]] — the pre-WP-05 block creating a Lead, a second `CustomerAccount`,
  a `Tenant` and a `Subscription` *before payment* was never removed, and
  `billing.service.ts` says so in a comment reading *"Until WP-07 moves tenant
  creation behind the payment…"*; and [[BUG-0078]] — `PROVISIONING_REQUESTED` is
  emitted with no consumer, so automatic provisioning has never run.

  The package's own title is "Payment to onboarding to provisioning automation".
  The emitting half shipped; the automation the title names did not. Nothing went
  red because no test asserts that an unpaid submission creates no tenant, and
  none asserts that an emitted event type has a handler.

  Remediation is [[TASK-0008]] WP-10 under
  [`EXECPLAN-0001`](../plans/EXECPLAN-0001-tenant-creation-behind-confirmed-payment.md).
  The row is marked `REOPENED` rather than rewritten to `NOT_STARTED`: the work
  that did ship is real and is depended on by WP-08 and WP-09.

<!-- GRAPH:BEGIN — generated by scripts/rebuild-tasks.mjs; edit the record, not this block -->

## Related

- Records — [[BUG-0070]], [[BUG-0077]], [[BUG-0078]]
