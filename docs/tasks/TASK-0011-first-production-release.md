...
- 2026-08-25 — closed. Both work packages were already `DONE`; only the header
  had been left `IN_PROGRESS`, and `NEXT_READY_WORK_PACKAGE: WP-02` named a
  package that was already complete — which is why `check-work-packages`
  computed NONE and disagreed with the field. Verified rather than assumed:
  `git merge-base --is-ancestor 270cde7 origin/main` confirms WP-01's merge
  reached production, and WP-02's release-history record exists.
