...
Handled as UX per the record's own retyping — confirmed again before touching
anything: no unauthenticated caller reaches page content on either code path,
so nothing here is an authorization fix. The premise (a hand-maintained list
missing real route trees) held, but the specific twelve had drifted: comparing
`PROTECTED_ROUTE_PREFIXES` against `readdirSync('app/(authenticated)/')`
showed `/onboarding` — one of the record's twelve — was already present (added
by unrelated work since the record was written), while `/access-denied` was
missing and not named in the record at all. Twelve names changed; the count
did not.

**Fix:**

- `PROTECTED_ROUTE_PREFIXES` (`apps/web/lib/auth-config.ts`) gained the twelve
  route trees actually missing: `/access-denied`, `/approvals`, `/benefits`,
  `/dlp-review`, `/employee-bank-accounts`, `/executive`, `/hr`, `/inbox`,
  `/loans`, `/manager`, `/my-preferences`, `/profile`.
- Per the record's secondary suggestion ("consider also passing the real
  pathname into `requireSessionUser()`"): the authenticated layout
  (`app/(authenticated)/layout.tsx`) no longer hardcodes `requireSessionUser("/")`.
  It reads the `x-dijipeople-pathname` header `proxy.ts` already stamps onto
  every request (the same header `generateMetadata` in the same file already
  reads for the page title) and passes the real path through. This makes the
  prefix list an optimisation rather than the only thing standing between a
  user and a lost deep link, exactly as the record asked — a route tree added
  to `app/(authenticated)/` without a matching prefix now loses its deep link
  only if the *proxy* passes it through with a session cookie present but no
  live session (a narrower window than "every unlisted route, always").
- The part of the proposed approach meant to outlive the twelve added strings:
  `apps/web/lib/auth-config.spec.ts` (new) walks the real
  `app/(authenticated)/` tree at test time and fails if any top-level route
  directory lacks a matching entry in `PROTECTED_ROUTE_PREFIXES` — so the next
  drift is a red test, not a fourteenth backlog item.

**Acceptance criteria:**

- Every route tree under `app/(authenticated)/` is covered — met, and now
  enforced by a test rather than by re-reading the directory by hand.
- An anonymous request to `/approvals` redirects to `/login?next=%2Fapprovals`
  — met (`isProtectedRoute` now true, and `proxy.ts:116-120` builds `next`
  from the real pathname whenever it is).
- An anonymous request to `/approvals/new` redirects to sign-in without
  running the page component — met, same code path.
- A test fails when a new authenticated route tree is added without a
  matching prefix — met, `auth-config.spec.ts`'s directory walk.

**Tests:** `apps/web/lib/auth-config.spec.ts` (new) — the directory-walk drift
guard, the twelve routes by name, and a regression guard on the `"/"` special
case in `matchesPrefix` (73 assertions, driven by the real route tree so the
count grows with the app). `npm --workspace web run test -- auth-config` — 73
passed. `npm --workspace web run test -- workspace-shell-headings
workspace-switcher-placement` (the other two suites touching the authenticated
layout) — 16 passed, no regression from the `requireSessionUser` change.
