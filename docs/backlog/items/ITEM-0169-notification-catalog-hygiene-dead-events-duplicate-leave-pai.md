...
- 2026-09-12 — resolved in SESSION-0103, sequenced after ADR-0009 as this
  record required. Six of the six acceptance criteria addressed, one of them
  partially, with the remainder split into [[ITEM-0174]]:
  - **Duplicate pair collapsed.** `LEAVE_APPROVED`/`LEAVE_APPROVAL_REQUEST` —
    confirmed to have zero trigger call sites anywhere in the codebase, unlike
    their dotted-form counterparts which are triggered from
    `leave.service.ts:771`/`:1851` — are marked `NOT_YET_AVAILABLE` and
    retired via a new `RETIRED_EVENT_ALIASES` map. The surviving
    `leave.request.submitted.approver`/`leave.request.approved.employee`
    entries gained EMAIL as a unioned default channel.
    `NotificationsRepository.migrateRetiredEventPreferences()` (called from
    both `bootstrapSystemDefaults()` and `seedNotificationConfig`) moves any
    existing tenant `NotificationPreference` row from the retired code onto
    its successor, idempotently, so no tenant's choice is orphaned.
  - **Dead entries marked, not hidden as Enabled.** `AUTH_OTP`,
    `leave.request.returned.employee`, `leave.request.escalated`,
    `employee.profile.change.submitted.hr`, `TIMESHEET_OVERDUE` — all
    confirmed to have zero triggers by grep — now carry
    `availability: 'NOT_YET_AVAILABLE'` and `enabledByDefault: false`; the API
    read model (`listPreferences`/`listRules`) reports them as such and
    rejects an attempt to enable them.
  - **`APPROVALS` → `APPROVAL` spelling.** `leave.request.escalated` was the
    only user of the `APPROVALS` spelling; retiring it as not-yet-available
    (above) removed the second spelling rather than renaming a still-dead
    entry.
  - **Generated descriptions rewritten.** Every `${name} workflow/payroll
    notification.` placeholder in the `TIMESHEET_*` and `PAYROLL_*`/
    `PAYMENT_BATCH_*`/`JOURNAL_*` groups now says when the notification is
    sent and to whom, in tenant language.
  - **Diagnostic text removed.** The `sourceOfTruth` string
    ("TenantSetting controls global lightweight notification toggles...")
    and the "read from the existing tenant settings API" blurb are gone from
    both the API response and the rebuilt `NotificationRulesManager` screen.
  - **Key convention — partially done, tracked separately as [[ITEM-0174]].**
    The two dead duplicate codes above are retired rather than renamed (they
    have no trigger to update). The remaining ~30 SCREAMING_SNAKE_CASE codes
    still in active use (`AUTH_*`, `PAYROLL_*`, `CLAIM_*`, `LOAN_*`,
    `TIMESHEET_*`, `PAYSLIP_*`, `BILLING_INVOICE_ISSUED`,
    `REPORT_SCHEDULE_DELIVERY`) were **not** renamed in this session: doing so
    means changing the literal eventKey/eventCode string at every call site in
    `auth.service.ts`, `claims.service.ts`, `loans.service.ts`,
    `timesheet-workflow.service.ts`, `payroll-operations.service.ts`,
    `payroll-journal.service.ts`, `payslips.service.ts`,
    `report-scheduler.worker.ts` and `super-admin.service.ts` — several of
    them modules this session was explicitly told not to touch (auth) or
    outside its assigned scope, and a rename attempted mid-session with other
    agents concurrently active on some of those same files was judged too
    large a blast radius to do safely. This is recorded rather than silently
    narrowed, per this task's instructions; ITEM-0174 carries the acceptance
    criterion this leaves open ("every catalog key follows one convention").
