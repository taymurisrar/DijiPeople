...
- 2026-08-15 — Applied and verified. `main` now requires a pull request, requires the `CI required gate` check, requires the branch to be up to date before merging, requires conversation resolution, dismisses stale approvals, and refuses force pushes and deletion — with `enforce_admins` enabled so none of it is bypassable. Verified by reading the protection object back from the GitHub API. Required approvals is 0 rather than the documented 1: this repository has a single maintainer, and GitHub does not permit self-approval, so requiring an approval nobody can give would block every merge including the one carrying this change. Raising it is a team-size decision and is recorded as an owner decision rather than silently assumed.
