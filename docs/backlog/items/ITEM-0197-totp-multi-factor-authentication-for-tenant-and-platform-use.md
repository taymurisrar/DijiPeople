...
Built by TASK-0032 WP-03 on `agent/pah-wp03-mfa` (merged as `f5b4b9d1`) per
ADR-0019: HOTP/TOTP implementation matching RFC 4226/6238 test vectors;
challenge-token-based login (no tokens/cookies issued until verified); pending
MFA setup stays pending until confirmed; recovery codes are single-use, keyed
hashes bound to the account; an administrator MFA reset is tenant/row-scoped;
no MFA secret ever reaches an audit row or a log line;
`PlatformUser.failedLoginAttempts`/`lockedUntil` landed in the same package,
closing [[BUG-3146]] alongside this. `mfaResult` in the login audit event now
reflects `NOT_REQUIRED | PASSED | RECOVERY_CODE_USED | FAILED`. Platform MFA
is optional by owner decision (ADR-0021) — no platform SETUP_REQUIRED flow
exists.

## QA Retest

Verified by TASK-0032 WP-09 live QA on both `apps/web` and `apps/admin`
(enrolment, sign-in with TOTP, sign-in with a recovery code, reuse refused,
setup refused without `SECRET_ENCRYPTION_KEY` — all Pass; see
`docs/tasks/TASK-0032-streams/QA-summary.md`) and the passing REG-537..REG-543
regression suite.
...
- 2026-09-25 — built on `agent/pah-wp03-mfa` (WP-03, merged `f5b4b9d1`, per
  ADR-0019); verified by WP-09 live QA; Architect disposition DONE.
