...
Deferred. TASK-0032 substantially improved coverage of both module families,
but not with the `services/api/test/` real-HTTP/real-database e2e suite this
item asks for:

- **Covered**: extensive new unit/integration specs across both areas
  (`partner-type-policy.spec.ts`, `partner-duplicate-detection.spec.ts`,
  `partners-audit.spec.ts`, `partner-experience-audit.spec.ts`,
  `lead-attribution-correction.spec.ts`, `placeholder-context.spec.ts`,
  `agreement-source-guards.spec.ts`, `contracts.agreement-guards.spec.ts`,
  `contracts.agreement-rendering.spec.ts`, `contracts.partner-source.spec.ts`,
  `contract-templates.seed.spec.ts`, and more — see the WP-04/WP-05/WP-11
  reports); a full live-QA pass (TASK-0032 WP-09) exercising both module
  families end to end through real HTTP calls and a real browser against a
  throwaway stack, including the two-signer signing lifecycle, immutability
  after signing, and the partner/lead commercial funnel from inquiry through
  activation.
- **Not covered — remains outstanding**: no new `services/api/test/*.e2e-spec.ts`
  was added for either module family. `contracts.e2e-spec.ts` and a
  partner/lead lifecycle e2e suite (this item's original Acceptance Criteria)
  do not exist, so neither area has a CI-run, database-backed e2e suite; the
  live-QA coverage above was manual/scripted against a throwaway stack, not a
  repo-tracked automated suite that runs on every push.

Deferred rather than closed: real progress was made, but the item's actual
ask (an automated, CI-running e2e suite) was not delivered, and building one
is a substantial, independent effort better scoped as its own task.
...
- 2026-09-25 — Architect re-triage: DEFER — unit/integration specs and live
  browser QA landed across TASK-0032's streams, but no DB-backed
  `services/api/test/` e2e suite was built; the original ask remains
  outstanding and is deferred as independent, substantial follow-up work.
