...
RelatedQA: 
RelatedADR: ADR-0011
RelatedImplementation: EXECPLAN-0038
TargetMilestone: 
BlockedBy: 
---
...
- 2026-09-12 — reviewed and scoped in SESSION-0103, sequenced after
  BUG-3375/ITEM-0171 as this record required. Full details in
  `docs/plans/EXECPLAN-0038-notification-coverage-for-silent-tenant-modules.md`.

  **A correction to this record's own evidence table first**: `support-cases`
  was listed as having no trigger. It has one —
  `SupportCasesService.sendCommunication` has called
  `EmailService.sendTemplateEmail({ eventCode: 'SUPPORT_CASE_UPDATE' })` since
  the module was written — but no catalog entry or template ever existed for
  that eventCode, so every call threw `BadRequestException` and the feature
  has never worked. Fixed by adding the catalog entry and a real system
  template using the variables the call site actually supplies
  (`notification-events.catalog.ts`); `support-cases.service.ts` itself was
  not touched. This closes the "support case updated" ranked candidate as a
  bug fix rather than new wiring.

  **The other four ranked candidates — document/policy acknowledgement due,
  contract renewal/expiry, recruitment stage change, business trip
  approved — were not implemented this session**, for lack of remaining
  session time, not because they were judged low value; each genuinely needs
  a new trigger written into its own domain module (`documents`, `contracts`,
  `recruitment`, `business-trips`), which is real cross-module work this
  session did not have budget to do safely for four modules at once. Split
  into [[ITEM-0173]] rather than silently dropped, with the ranking reasoning
  preserved.

  **The six lower-ranked modules, reviewed and declined for now**, each with a
  reason so a future survey does not re-raise them without cause:
  - `benefits` — enrolment changes are typically annual/open-enrolment events
    an HR admin drives in bulk; a per-employee notification was judged more
    likely to be noise than signal without a specific trigger event named by
    a product decision.
  - `compensation` — a compensation change is sensitive HR data; notifying by
    email risks the same class of exposure `sensitive-field-overexposure`
    names, and the right channel (in-person, HR-mediated) is arguably not
    email at all. Needs a product decision, not an engineering default.
  - `projects` — no single state transition in this module reads as
    obviously notification-worthy the way an approval or a deadline does;
    revisit if a specific request names one.
  - `sla` — SLA breach escalation already exists at the `leave`/`attendance`
    approval level via `NotificationRecipientResolverType`-driven rules; a
    generic cross-module SLA notification would duplicate that rather than
    add coverage.
  - `legal` — legal document publication/acknowledgement already has its own
    workflow (`docs/architecture/` legal module) with an internal
    acknowledgement gate; whether it should also email is a product decision
    outside this survey's remit.
  - `partners` — the partner-experience module already has its own
    communications surface (`partner-experience`); adding a second,
    generic-catalog path risks the exact "two paths, one gated" shape
    ITEM-0171 just closed for payroll/auth.
