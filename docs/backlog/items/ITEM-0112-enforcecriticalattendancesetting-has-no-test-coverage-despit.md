...
Premise partially stale, confirmed before implementing: `enforceCriticalAttendanceSetting`
was no longer *entirely* uncovered. `attendance-settings-mandate.spec.ts` — a
file that names this exact item in its own header comment — already exercised
every mandated key, the `category !== 'attendance'` early return, and the
no-op/rewrite behaviour, all through the public `updateTenantSettings` entry
point. What the record's acceptance criteria specifically asked for was still
missing:

- `grep -rn "enforceCriticalAttendanceSetting" services/api/src --include=*.spec.ts`
  still returned nothing — the function was tested only by consequence,
  through `assertAttendanceSettingIsChangeable`, which the same map backs and
  which runs first and throws before `enforceCriticalAttendanceSetting` is
  ever reached for a contradicting value.
- The mandated set was re-listed by hand in the spec's `MANDATED` table rather
  than checked against an exported constant — exactly the "mutation-testing
  failure this repository has hit before" the record warns about: a key added
  to the map without being added to `MANDATED` would pass every existing test.

**Fix:** exported `MANDATORY_ATTENDANCE_SETTINGS` and
`enforceCriticalAttendanceSetting` itself from `tenant-settings.service.ts` (both
were module-private). Added a new `describe` block to
`attendance-settings-mandate.spec.ts` that imports and calls the function
directly — so deleting it is a compile error, not a silent pass — covering:
every mandated key rewritten regardless of input, a non-mandated attendance key
passed through unchanged, the `category !== 'attendance'` early return using a
mandated key *name* in a different category, and a parity check
(`Object.keys(MANDATORY_ATTENDANCE_SETTINGS)` against the independent
`MANDATED` table's key list, with the one array-valued key —
`locationRequiredForModes`, covered by its own pre-existing tests — named
explicitly as the reason it is not in `MANDATED`) so a key added to the map
without a decision about its test coverage fails a test rather than shipping
silently.

**Acceptance criteria:**

- A spec file references `enforceCriticalAttendanceSetting` by name — met.
- Deleting the function fails at least one test — met, a compile-time failure
  since the new block imports it directly.
- Removing a key from the map fails at least one test — met, both via the
  parity check and via the pre-existing `assertAttendanceSettingIsChangeable`
  tests.
- Changing a mandated key's value fails at least one test — met, both new and
  pre-existing tests assert exact values.
- The `category !== 'attendance'` early return is covered — met, both for the
  assertion (pre-existing) and for `enforceCriticalAttendanceSetting` itself
  (new).

**Tests:** `npm --workspace api run test -- attendance-settings-mandate` — 37
passed (13 pre-existing test cases plus the new direct-coverage block).
