...
REG-527 (`services/api/src/modules/auth/auth-activity-authorization.spec.ts`,
...
Proven to fail against the pre-fix code: 17 of 20 tests in
`auth-activity-authorization.spec.ts` fail when reverted to `10d5d148` (all 14
non-elevated platform roles, the tenant user, the metadata test and the
no-user test).
...
Fixed by commit `01672ca2` on `agent/pah-wp02-rbac` (TASK-0032 WP-02, merged
as `d5535f0a`): `POST /auth/activity` is now marked `@AuthenticationOnly()`
(no business permission, session presence only) and
`PermissionsGuard` honours the marker before reading permission metadata;
`apps/admin`'s error interceptor now has an explicit background-call rule so a
failed heartbeat never opens the blocking modal.

## QA Retest

Verified by TASK-0032 WP-09 live QA against the throwaway stack and the
passing `auth-activity-authorization.spec.ts` / `background-request.spec.ts`
— see `docs/tasks/TASK-0032-streams/QA-summary.md` ("Admin screens …" rows: no
failed calls, no console errors).
...
- 2026-09-25 — fixed at `01672ca2` (WP-02); verified by WP-09 live QA;
...
- Regression — REG-527 (see the regression register)
