...
Fixed in two places, because either alone would leave the defect reachable.

**The emitter.** The checkout-unavailable panel in
[`subscribe-form.tsx`](../../apps/landing/app/subscribe/subscribe-form.tsx) now
links to `/contact?checkout=<code>` instead of the partner referral parameter.

**The capture layer.** [`referral.ts`](../../apps/landing/lib/referral.ts) rejects
the platform's own diagnostic-code shape (`DP-<letters>-<digits>`) on **both**
the write path and the read path. The read path matters as much as the write
path: every visitor who clicked the old link is already carrying
`dijipeople_referral=DP-CHK-01` with up to thirty days left on it, and guarding
only capture would leave that cohort attributing to an error code *and* still
blocking partners — because capture bails as soon as `readReferralCode()`
returns anything at all. Treating a stored diagnostic as absent heals them.

Regression coverage: nine cases in `referral.spec.ts`, including near-miss codes
(`DPCHK01`, `DP-CHK-01-X`, `DP-1`, `DPARTNER-2`) that must still be accepted —
a guard that ate real partner codes would be the same class of revenue defect.
Proven to fail without the fix: `Tests: 5 failed, 4 passed`.

## QA Retest

Verified in `docs/qa/runs/2026-08-25-landing-fixes-verification.md` (V4–V9). The decisive case is V5: after a diagnostic code
has been seen, a genuine partner code is stored — which is what production does
not do today.

Guarded by scenario [[QA-LANDING-018]], which flips from FAIL to PASS.
...
- Regression — REG-253 (see the regression register)
