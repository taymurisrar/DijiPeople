...
Fixed 2026-08-28 on `agent/open-bug-sweep`, with [[BUG-1425]] as this record
asked — either half alone leaves the other open.

Frontend: the partner form's `currencyCode` is an `option` over
`PLATFORM_CURRENCY_OPTIONS`, which is how contracts already declared the same
field. The `"currency"` control keeps its single meaning, a money amount. The
audit this record asked for found exactly three declarations of `currencyCode`
in the registry: partners (numeric, wrong), contracts (already an option), and
commissions (plain text) — the last is now an option too.

API: see [[BUG-1425]]. `@IsIn(PLATFORM_CURRENCY_CODES)` replaces
`@MaxLength(3)`, so `"5"` is rejected at the boundary whichever client sends it.
...
`services/api/src/modules/partners/dto/partner-currency.spec.ts` (17 assertions)
and `packages/config/platform-currencies.test.js`.

**For whoever retests:** partners created through the old form still hold
`currencyCode: "5"`. Nothing here repairs stored rows, and an operator editing
such a partner will now be asked to pick a real currency before the record will
save. That is intended, but it is a change an operator will notice, and it is
the reason this is worth a browser pass rather than a code read.
...
- 2026-08-28 - Currency is a select over the supported codes, and the API validates the code rather than its length. REG-273.
