...
A spec that drives `apiRequest` with no access cookie and a refresh that returns
`401`, asserting that no fetch to the API is made without Authorization and that
the raised error carries the revoked reason. Registered as a regression entry
once written.
