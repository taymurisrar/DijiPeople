...
## QA Retest
Retested 2026-08-29 by the regression-guard sweep: `apps/web/app/components/data-table/empty-state-message.spec.ts` ran and passed, as part of `npm --workspace web run test` (888 passing).
...
Not verified. QA-TENANT-019 is written against an empty list in a TENANT
workspace, which needs a tenant login this pass did not have.

What the pass did establish is that this fix is **app-scoped**. `apps/web`
has the filter-aware `emptyStateMessage()` at
`apps/web/app/components/data-table/utils.ts:277`, covered by
`empty-state-message.spec.ts`. `apps/admin` renders a static
"Create a ... or adjust the current view and filters." for every module
regardless of filter state — confirmed in a browser on Admin > Support cases
with no filters set.

That sibling defect is recorded as [[BUG-1752]] rather than reopening this
record, since the fix here was correct for the app it targeted.

## History

- 2026-08-27 — found on the first browser sign-in to a newly provisioned tenant
- 2026-08-28 — tenant retest still blocked; the same defect found unfixed in `apps/admin`, recorded as [[BUG-1752]].
  workspace.
...
- `apps/web/app/components/data-table/empty-state-message.spec.ts`
