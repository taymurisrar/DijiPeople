...
A unit test on `getClientAccessTokenTtl` asserting a bare `"1800"` input
normalizes to an effective 1800-second lifetime, failing against the unfixed
code (which currently returns `"1800"` verbatim, producing a ~1-second
token). No `REG-nnn` entry yet.
