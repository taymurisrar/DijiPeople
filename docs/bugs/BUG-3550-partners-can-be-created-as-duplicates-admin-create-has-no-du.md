...
A unit test creating two partners with the same `taxId` via the admin path,
asserting the second is refused/flagged, failing against the unfixed code.
Another asserting `submitInquiry` catches a `taxId`-only match. No `REG-nnn`
entry yet.

## Dependencies

A decision on whether either field becomes a DB-level unique constraint
(schema change) or an application-level check only (see Proposed Resolution
point 3).
