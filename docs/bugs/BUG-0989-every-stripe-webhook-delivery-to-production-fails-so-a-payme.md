...
**No code change.** The owner set `STRIPE_WEBHOOK_SECRET` on
`srv-d7js7fqqqhas739v4i7g` to the signing secret of the `dijipeople` event
destination — the endpoint at
`https://api.dijipeople.com/api/billing/stripe/webhook`, `Active`, in the
`DijiPeople Test` sandbox on account `acct_1U72MgHSlnE5ArNF`. The change
triggered deploy `dep-da68nme1egvs739pn1ag`, live at 2026-08-24T18:28:57Z.

The root cause stands as diagnosed: the secret the service verified with was not
the secret Stripe signed with. Everything else in the path — the raw-body
pipeline in `configureBodyParsing`, the header, `constructEvent` — was correct
throughout.

## QA Retest

**Verified 2026-08-24.** Three independent signals, which matters because no one
of them is sufficient alone.

**1. Stripe's own record.** The owner resent a previously failed delivery from
the Event deliveries list:

| | |
|---|---|
| Event | `price.created`, `evt_1U7j6pHSlnE5ArNF4Lo0aYgq` |
| Origin date | 2026-08-24 02:39:19 PKT — one of the failures |
| Attempt date | 2026-08-25 00:11:10 PKT, *Resent manually* |
| Delivery status | **Delivered · Recovered** |
| Response | **HTTP status code 200** |

This is Stripe reporting what the server returned to a genuinely signed request,
which is the one thing no probe from this side could establish.

**2. The error log stopped.** Counting `stripe/webhook` error lines in the
service log either side of the deploy:

| Window | Lines |
|---|---|
| 2026-08-23T20:00Z → 2026-08-24T18:28Z (before) | **22** |
| 2026-08-24T18:30Z → onward (after) | **0** |

**3. Nothing was logged for the successful delivery**, and that is the expected
shape rather than a gap: a `200` produces no `HttpExceptionFilter` warning, and
`LOG`-level lines are filtered by the service's `LOG_LEVEL` — the same filtering
that made [[BUG-0904]]'s log-line criterion unsatisfiable.

**A hypothesis worth recording because it was wrong.** During diagnosis it was
raised that the endpoint might live in a *sandbox* rather than plain test mode,
and that a cross-environment secret would fail identically while looking correct.
The Webhooks tab settles it: the endpoint does live in the `DijiPeople Test`
sandbox, and production's secret now matches *it*. The concern was well-founded —
a sandbox and a test-mode endpoint carry different signing secrets — and simply
did not apply here.

### What remains untested

The acceptance criterion *"a test-mode checkout run end to end produces a
`StripeWebhookEvent` row, a confirmed order, a `PROVISIONING_REQUESTED` outbox
row, and a provisioned tenant"* is **not** met by this retest. A `price.created`
event proves signature verification; it does not exercise provisioning. That run
is [[ITEM-0078]] and remains outstanding.

Closed on the defect as filed — *every delivery fails* — which is no longer true.
...
- 2026-08-24 — **fixed and verified.** Secret corrected by the owner; a manually
  resent delivery returned `200 Delivered · Recovered`, and the 22 error lines
  that preceded the change became 0. `VERIFIED`.
...
- Modules — [[billing]]
- Regression — REG-245 (see the regression register)
