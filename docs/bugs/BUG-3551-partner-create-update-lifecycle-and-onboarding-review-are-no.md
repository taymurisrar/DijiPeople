...
REG-555 (`services/api/src/modules/partners/partners-audit.spec.ts` — create,
update, lifecycle transition) and REG-556
(`services/api/src/modules/partner-experience/partner-experience-audit.spec.ts`
— inquiry qualify/reject, onboarding review, partner activation, partner-lead
review). Both proven to fail against the pre-fix code — neither service had
an `AuditService` dependency before this branch.
...
merged as `7f134e70`): `AuditService` is now injected into both
`PartnersService` and `PartnerExperienceService`, and every mutation this
record lists writes an `AuditService.log()` call (`tenantId: 'platform'`)
alongside the existing `PartnerTimeline` write.

## QA Retest

Verified by TASK-0032 WP-09 live QA (partner lifecycle actions confirmed
audited during "Partners …" and "Lead attribution …" scenarios — see
...
- 2026-09-25 — fixed at `5dfb4de4` (WP-04); verified by WP-09 live QA;
...
- Modules — [[partners]]
- Regression — REG-555 (see the regression register)
