...
Fixed across four new/changed files under `apps/web`.

1. Added `apps/web/app/(authenticated)/settings/subscription/loading.tsx`
   (skeleton, matching the `employees/loading.tsx` convention) and
   `.../error.tsx` (retryable, matching `employees/error.tsx`). The nested
   `(authenticated)/error.tsx` already existed and handles a thrown error for
   the whole authenticated section; this one is scoped to the subscription
   segment and shows a route-specific message sooner. It does not replace the
   `LoadFailureState` below — `loadSubscriptionSettingsData` catches its own
   request errors and returns `{ ok: false }` rather than throwing, so a
   caught error never reaches either `error.tsx` boundary.
2. New `apps/web/app/(authenticated)/settings/subscription/_components/load-failure-state.tsx`.
   `SubscriptionSettingsPage` (`_components/subscription-settings-page.tsx`)
   now renders this — role="alert", a trace id, and a `router.refresh()`
   retry button — for the `!subscriptionData.ok` branch, and keeps
   `AccessDeniedState` only for the actual permission refusal.
3. The gate is now `hasSettingsPermission(user, PERMISSION_KEYS.BILLING_VIEW)`
   (`apps/web/app/(authenticated)/settings/_lib/require-settings-permission.ts`,
   already used by every other settings screen) instead of
   `hasElevatedTenantRole(user?.roleKeys)`. This is not full parity with the
   API's `TENANT_ADMINISTRATION:read` matrix privilege — no entity-key mirror
   for that exists anywhere in `apps/web` yet, on any screen — but it closes
   the specific gap this record measured: a user holding `billing.view` but
   not an elevated role is no longer refused by the UI.
4. `load-subscription-settings.ts`'s `loadSubscriptionSettingsData` now takes
   `activeView` and only requests `/billing/invoices` when
   `activeView === "billing-history"`, resolving to an empty list otherwise.
   It also now reads and returns `availableCurrencies` from the plans
   response (used by BUG-3333's UI half), read defensively (empty array
   rather than a thrown error) since it is a new, optional field.

Regression coverage: `subscription-settings-page.spec.ts` (source-level —
`apps/web` has no jsdom).

## QA Retest

Pending — needs confirmation that a genuinely failed `GET /billing/plans` (or
`/subscription`) renders the retryable load-failure state rather than
access-denied, that retry actually reloads the data, and that a user holding
only `billing.view` (no elevated role) can now reach the screen.
...
- Regression — REG-417 (see the regression register)
