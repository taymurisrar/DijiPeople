...
Not applicable — not yet fixed. Verified by reading both handlers in full at
`1af3690`. **No request was executed against a running API**, so the API's `403`
on those paths is inferred from the handler's own branch existing — the branch
