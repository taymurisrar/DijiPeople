...
Re-confirmed live against production `e0aeabcd` on 2026-08-28.

Inspecting every control on **Leads > New Lead** returned, for all six inputs:

```
ariaLabel: null    ariaLabelledby: null
hasIdLabel: false  wrappedInLabel: false
```

A DOM query for `main label` returns an empty list — the runtime emits
**no label elements at all**; the visible field captions are non-label
elements. Inputs carry no `name` and no `id`, so nothing can associate a
caption with a control. Custom dropdowns are plain buttons with no
`role="combobox"`.

By contrast the data tables ARE labelled — row checkboxes carry
`aria-label="Select row 1"` — so the gap is specific to the runtime form
layer, which narrows the fix.

No new record filed; this is evidence on the existing one.
