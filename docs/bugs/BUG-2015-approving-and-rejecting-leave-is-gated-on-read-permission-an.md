...
None yet. `services/api/test/permission-propagation.e2e-spec.ts` is the suite
`AGENTS.md` names for this class of change and is the right home: a case that
grants read without approve and asserts a 403 on the approve endpoint would fail
today. `common/constants/wiring-invariants.spec.ts` is the second place to look —
an invariant that no state-changing route declares only a `read` privilege would
catch the whole class rather than this instance.
...
Awaiting a fix — nothing to retest yet. The retest is the e2e case described
under Regression Coverage rather than a manual attempt, because the environment
cannot produce the second activated user a manual attempt needs.
