...
QAReport: 
RegressionId: REG-482
RelatedBacklogItem:
RelatedDecision:
RelatedImplementation: [docs/plans/EXECPLAN-0045-customization-end-to-end-for-permission-holders.md, apps/web/app/(authenticated)/settings/customization/_lib/column-payload.ts, apps/web/app/(authenticated)/settings/customization/_components/columns-management.tsx, services/api/src/modules/customization/customization.service.ts, services/api/src/modules/customization/dto/customization.dto.ts]
...
REG-482, QA scenario QA-SETTINGS-022:

- `services/api/src/modules/customization/column-payload.seam.spec.ts` transpiles
  the real web `column-payload.ts` and sends choice, reference (Employees), blank
  text, number, datetime and boolean payloads through a `ValidationPipe`
  configured like `main.ts` and `CustomizationService.createColumn`; a text
  length of 0 is rejected; a legacy `maxLength: null` is accepted.
- `apps/web/app/(authenticated)/settings/customization/_lib/column-payload.spec.ts`.

Mutation-checked: server treating `null` as below 1 (1 failed); client sending
`null` with the server rejecting it, the original seam defect (7 failed); client
sending a length for every type (3 failed). The requirement below is met by the
seam spec; no DB-backed e2e was added.
...
merged into `agent/walkthrough2-integration`; plan EXECPLAN-0045). One rule for
an absent length, enforced at the API and honoured by the client.

- **Client.** The Add field payload is built by one import-free module,
  `apps/web/app/(authenticated)/settings/customization/_lib/column-payload.ts`,
  which omits `maxLength` when the type has no length or the input is blank, and
  never sends `null` for optional fields.
- **Service.** `validateValueRules` treats `null` and `undefined` alike as "no
  length"; the length rule applies only to text, textarea, email, phone and URL
  fields; `buildColumnData` stores `null` for other types. A text length of `0`
  is still refused (`@Min(1)`). The DTO type is `number | null`.
- **Dialog.** Maximum length is shown only for length types, and errors render
  on their field.
- A stale client still sending `maxLength: null` is accepted.

Residual (harmless): an old non-length column that stored a length keeps it
until edited.
...
scenario S2) on a throwaway database: a text, a choice and a reference field
each created with 201. Not separately re-run on production beyond the general
release verification in WP-08. CI runs 34732185363, 34732682935 and 34732697734
passed on f865ac5e (the merged tree). Scenario QA-SETTINGS-022:

1. Module → Fields → Add field: create Choice (two options), Reference →
   Employees, Number, Date and time, Yes/No, and Text with a blank maximum
   length. Each returns 201 and appears with a human-readable type.
2. Text with maximum length `0` shows the error on the Maximum length field.
