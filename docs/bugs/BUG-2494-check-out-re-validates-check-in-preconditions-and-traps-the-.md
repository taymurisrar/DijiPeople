...
Fixed in `ade1fea7` (2026-08-30), verified against this record during the
2026-09-11 triage sweep rather than fixed by it — the fix and this record were
authored in the same commit, but `Status`/`ArchitectDisposition` were left at
`OPEN`/`TRIAGE_REQUIRED` pending the Architect's sign-off, which this update
supplies.

The `validateModeAndLocation` call inside `checkOut`
(`services/api/src/modules/attendance/attendance.service.ts`) was removed
entirely, exactly as proposed: check-out now validates only what
`validateAttendanceLocationPayload` checks — the location payload the client is
submitting *now* — and never re-runs the check-in gate against the stored
`attendanceMode`/`officeLocationId`. A load-bearing comment at the call site
records why the check must never come back, quoting the production entry that
was trapped.

Verified against this triage:

- `checkOut` closes an entry with `attendanceMode: OFFICE` and
  `officeLocationId: null` — the exact production state
  (`attendance.service.spec.ts:714`, "closes an OFFICE entry whose work site is
  no longer on the record").
- `checkIn` still refuses `OFFICE` without a resolvable work site — the
  precondition is enforced where it belongs, unchanged.
- The device-location control on check-out (`validateAttendanceLocationPayload`)
  is untouched and still required.
- The three routes the Root Cause section names — `onDelete: SetNull` on
  `officeLocation`, a disabled mode in policy, and the read path's own
  `officeLocation ?? employee.location` fallback — no longer matter to
  check-out, because check-out no longer inspects the stored mode/location at
  all. Nothing further to close on any of the three.

The single production entry named in Evidence
(`85303ef3-4285-45d4-a751-370a00a78828`) closes itself the first time the
affected employee calls check-out after this fix deploys; no manual data repair
is needed.

No further code change made as part of this triage pass — this record was
re-verified, not re-implemented.

## QA Retest

Regression coverage confirmed present and passing:
`services/api/src/modules/attendance/attendance.service.spec.ts` — "closes an
OFFICE entry whose work site is no longer on the record" and "requires a fresh
device location again at check-out", plus the pre-existing check-in refusal
assertions. `npm --workspace api run test` — full suite green (see the
task-level validation summary). REG-372 registered
...
- 2026-09-11 — re-verified during the FIX_NOW triage sweep. The fix, its
