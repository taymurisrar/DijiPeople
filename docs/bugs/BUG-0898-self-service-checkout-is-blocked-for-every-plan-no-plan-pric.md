...
defect.** It is the pricing half of [[BUG-0903]] and the two are one act.

This record's own diagnosis says the mechanism is sound: *"The mechanism is not
broken; it has never been run."* Syncing prices to Stripe is an operator action
against a real Stripe account, and `seed-commercial.ts` deliberately refuses to
do it — *"not something a seed should do on anyone's behalf."* A thing nobody
has chosen to do yet is not a defect.

**The measured figure in the Summary is also out of date.** It says 0 of 36
plan prices carry a synced Stripe price. Measured against production on
2026-08-28 via `GET /api/public/plans`: 4 plans, 18 public prices, **8 synced**
and checkout-ready, 10 not synced — all 8 under `stripeEnvironment = TEST`.

So the current state is narrower than recorded: partial TEST-mode coverage
rather than none. It changes nothing about the decision, because TEST-synced
prices are not sellable under live mode and all of them need re-syncing after
the mode switch either way.

## QA Retest

Not applicable until the decision is made.

The measurement to repeat afterwards is the one used here:

```
curl -s https://api.dijipeople.com/api/public/plans
```

Every public price should report `stripeSyncStatus: SYNCED`,
`stripeEnvironment: LIVE` and `checkoutReady: true`. Anything still `TEST` after
a live switch is a price that will refuse to sell.
...
- 2026-08-28 - reclassified PRODUCT_DECISION at the owner's direction. Also corrected the count: 8 of 18 prices are synced (TEST), not 0 of 36.
