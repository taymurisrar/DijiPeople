...
`apps/web/lib/server-api.spec.ts` drives `apiRequest` with a `next/headers`
...
Fixed 2026-09-12, in `apps/web/lib/server-api.ts`. Added
`canPersistCookies()`, which probes whether the current `next/headers`
cookie store can actually be written (a real, harmless `cookieStore.set()` of
a dedicated probe cookie name with delete semantics, wrapped in `try/catch`)
and is called **before** a refresh is attempted, at both the pre-emptive
refresh site and the retry-after-401 site. When it returns `false`, refresh is
skipped entirely — the request instead falls through to [[BUG-3356]]'s
session-ended handling, and the refresh token in the browser's cookie is never
consumed, so the middleware or a route handler can still use it.

`persistRefreshedAuthCookies` no longer swallows a write failure silently: its
`catch` now logs via `console.error` (there is no server-side logger in this
app) and the function returns `boolean` so its caller can react — if tokens
were issued but somehow still failed to persist (a residual defensive case,
since `canPersistCookies()` should have already ruled this out), `apiRequest`
treats it as a failed refresh rather than proceeding with a token the browser
will never see.

The alternative resolution named in the bug record — a short reuse window
tolerating an unpersisted rotation — was implemented instead as part of
[[BUG-3359]]'s grace window, since that record needed the same tolerance for
a different reason (a concurrency race, not an unwritable context) and one
...
- 2026-09-12 — fixed on `agent/r-s3-auth`: the unwritable cookie store is
  detected before a refresh is attempted, not after, so a Server Component
  render never spends a refresh token it cannot persist.
