...
Fixed, and narrower than the record's "make it a seeded fact" option — that
remains the durable improvement and is not done here.

What changed is the decision rule. `isUsableLookupList` in
[`use-country-options.ts`](../../apps/landing/lib/use-country-options.ts) now
requires the lookup to be **at least as complete as the bundled list** rather
than merely non-empty. Production's eight-country answer no longer beats the 31
countries compiled into the page, so a buyer outside those markets keeps a
usable list. Widening to the full ISO set is unaffected, which is the case the
lookup exists for.

The silent-failure half is also addressed: `syncCountriesIfNeeded` now logs at
`error` with the surviving row count and a hint that a count near
`DEFAULT_COUNTRIES.length` means the ISO set has never loaded. The failure is
still swallowed — a reference lookup must not block a purchase — but it is no
longer invisible.

Not done: moving the country set into seeds. That needs an ExecPlan and is left
as the follow-up the record already describes.

## QA Retest

Verified in `docs/qa/runs/2026-08-25-landing-fixes-verification.md` (V10–V11). Eight tests, including the exact production
shape (a remote answer of 8) and the full ISO set (250).

Note the production endpoint still returns eight; this fix makes that harmless
to the buyer rather than correcting the server. Re-check the endpoint after
deployment.
...
- Regression — REG-254 (see the regression register)
