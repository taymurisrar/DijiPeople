...
Fixed exactly as the record specified, in
`apps/web/app/components/runtime/responsive-runtime-tabs.tsx`:

- The measurement copy now carries `inert`, removing it from the focus order
  the same way `aria-hidden` already removed it from the accessibility tree.
- The visible strip now exposes real tab semantics: `role="tablist"` (with
  `aria-label` and `tabIndex={-1}`, since it owns the arrow-key handling but
  is not itself a page-level Tab stop) on the container, `role="tab"` with
  `aria-selected` and `aria-controls` on each visible tab, roving `tabIndex`
  (`0` on the selected tab, `-1` on the rest), and arrow-key movement
  (`ArrowLeft`/`ArrowRight`/`Home`/`End`) that skips disabled tabs and wraps
  at both ends. The arrow-key math is a pure exported function
  (`resolveNextTabIndex`) so it has a regression test independent of
  rendering.
- `apps/web/app/components/metadata/runtime-metadata-form-renderer.tsx` now
  renders the panel with `role="tabpanel"`, an `id`, and `aria-labelledby`
  pointing at the selected tab's id — only when the tab strip is actually
  mounted (more than one tab), so a single-tab or tab-less form does not
  claim tab semantics it does not have. `getResponsiveTabId`/
  `getResponsiveTabPanelId`, exported from the tab-strip component, are the
  one place both components derive these ids from, via a shared `idPrefix`
  (`useId()`) so two mounted forms on the same page cannot collide.
- The More menu is untouched and stays reachable by ordinary Tab order
  immediately after the tab strip.
...
is green, including new source-structural and pure-function regression tests
(`apps/web`'s jest has no jsdom, so a rendered accessibility tree could not be
inspected directly). See REG-421 and QA-RUNTIME-042 for what a future QA pass
should re-run in a real browser, including the automated accessibility scan
this record's own Acceptance Criteria call for.
...
- Regression — REG-421 (see the regression register)
