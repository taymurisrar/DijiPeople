...
REG-525 (`services/api/src/modules/super-admin/super-admin.service.spec.ts`,
"BUG-3544 tenant profile edit is authorized by platform permission";
`services/api/src/modules/platform-auth/platform-permissions.spec.ts`, "ADR-0018
platform route authorization"). Also covered by REG-526 (platform routes
gated by tenant role keys through `RolesGuard`) and REG-529 (Super Admin is
not a bypass: platform-user management by permission, `RolesGuard`
tenant-only) — the same discovery pass found the identical role-key-as-gate
idiom repeated across `super-admin`, `leads`, `legal`, `demo-data` and
`platform-users`. Proven to fail against the pre-fix service: 9 of 16 tests in
`super-admin.service.spec.ts` fail when reverted to `10d5d148`.
...
Fixed by commits `bc2f9592`, `63ebc122` and `d58f1330` on
`agent/pah-wp02-rbac` (TASK-0032 WP-02, merged as `d5535f0a`): the
`isSystemAdmin` tenant-role-key check is replaced by the platform permission
model `assertModuleWrite` already enforces; the identical idiom on
`super-admin.controller.ts`'s direct REST route is fixed the same way; and
`RolesGuard` no longer relies on the `platformAccessForRole` alias to admit
`PLATFORM_OWNER`.

## QA Retest

Verified by TASK-0032 WP-09 live QA — see
`docs/tasks/TASK-0032-streams/QA-summary.md` ("Platform RBAC … Tenant profile
edit … Before: 200, 200, 403, 403. After: 200 for all four"; "Tenant edit in
the admin UI as PLATFORM_ADMIN and PLATFORM_OPERATIONS": "Tenant saved.", no
permission dialog) and the passing `super-admin.service.spec.ts` /
`platform-permissions.spec.ts`.

## History

- 2026-09-25 — created from qa run at `75fec5b9`; discovery stream D1.
- 2026-09-25 — Architect triage: FIX_NOW (TASK-0032).
- 2026-09-25 — fixed at `bc2f9592`/`63ebc122`/`d58f1330` (WP-02); verified by
  WP-09 live QA; Architect disposition DONE.
