...
The `contracts` portion is now covered: REG-570
(`services/api/src/modules/audit/lifecycle-audit-coverage.spec.ts`'s new
`ContractsService` entry; `contracts.agreement-guards.spec.ts` — "create()
writes a platform audit row for the new agreement") and REG-571 (public
signing events — `completeSignature`/`declineSignature`/
`requestSignatureChanges` — now audit with a null platform actor). Both
proven to fail against the pre-fix code (31 mutating endpoints, zero
`AuditService` calls).

`customization` (33 endpoints), `users.update` and role creation remain
**unaddressed** — `lifecycle-audit-coverage.spec.ts` still enumerates only
`EmployeesService`/`OrganizationService`/`LeaveService`/`ContractsService`;
the Proposed Resolution's "enumerate every module" step was not done. This
record stays open for those three areas.
...
Partially resolved by TASK-0032 WP-05 (`agent/pah-wp05-agreements`, merged as
`2204cd75`): the `contracts` module's 31 mutating endpoints — the highest-
severity portion cited in Impact ("a signed employment contract can be
amended with no attributable record") — now call `AuditService.log()`
alongside every existing `ContractTimeline` write, including public signing
events with a null platform actor. `customization`, `users.update` and role
creation are unchanged and remain open; this record stays `DEFERRED` for that
remaining scope rather than closing, since three of its four named areas are
still exactly as described in Actual Behavior.

## QA Retest

The `contracts` portion is verified by the passing
`lifecycle-audit-coverage.spec.ts` and `contracts.agreement-guards.spec.ts`;
no QA run yet covers `customization`/`users`/`roles` auditing, which remains
outstanding.

## History

- 2026-09-10 — created from the 2026-09-10 full technical audit (OBS-17) at `f36749b3`.
- 2026-09-25 — TASK-0032 WP-05 audited the `contracts` portion (merged
  `2204cd75`); `customization`/`users.update`/role creation remain
  unaddressed and this record stays `DEFERRED` for that scope.
