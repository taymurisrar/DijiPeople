...
Not yet fixed. The product decision is made — see
[ADR-0006](../decisions/ADR-0006-product-decisions-from-the-2026-09-11-backlog-review.md),
Decision 2: Option 1 of this record's own Proposed Resolution — the platform
path gets its own server-side policy, equivalent to the tenant path's
`allowRememberMe`. The refresh TTL is capped well below thirty days (the exact
value is an implementation choice within that bound, to be fixed and recorded
when this is built), and — folding in `ITEM-0108`'s companion question — the
session limit both surfaces observe is idle-based, not absolute, and must warn
before it expires. Disposition moves to `PLAN_REQUIRED`: what remains is the
policy store, the TTL value and the idle-timer/warning mechanism, and revising
`auth-session-lifecycle.spec.ts` to assert the decision instead of pinning the
accident it was written to pin.
...
- 2026-09-11 — answered by the product owner: refusable-by-policy remember-me,
  refresh TTL capped well below 30 days, idle-based session limit with a
  warning before expiry. Recorded in ADR-0006 (Decision 2), together with
  ITEM-0108. Disposition moves to PLAN_REQUIRED.
