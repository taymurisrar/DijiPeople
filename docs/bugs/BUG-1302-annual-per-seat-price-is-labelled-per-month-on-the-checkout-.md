...
Fixed. The estimate sentence moved out of the JSX template literal into
`formatSeatTotalEstimate` in [`apps/landing/lib/plans.ts`](../../apps/landing/lib/plans.ts),
beside `formatBillingUnit` — which already branched on `billingCycle` correctly,
and is the precedent this now follows. The period is derived from the price
rather than assumed, and the two functions are asserted to agree.

`subscribe-form.tsx` calls the helper and keeps its "Billed as one
subscription." fallback for flat prices via `?? `.

Regression coverage: six cases in `plans.spec.ts`. Proven to fail without the
fix — reintroducing the hardcoded literal produced `Tests: 2 failed, 11 passed`.

## QA Retest

Verified in `docs/qa/runs/2026-08-25-landing-fixes-verification.md` (V1–V3) on the running product: the live page renders
`$3 · 25 purchased seats · estimated $75.00 per year.` for the annual Starter
selection that previously said "per month", and the monthly case is unchanged.

Guarded by scenario [[QA-LANDING-017]], which flips from FAIL to PASS.
...
- Regression — REG-252 (see the regression register)
