...
choosing the first option the record proposed: a self-service route, not
removal.

**The selector.** `AttendancePolicyResolverService.resolveAuthorizedWorkSiteOptions`
(new method, `services/api/src/modules/attendance-engine/attendance-policy-resolver.service.ts`)
returns `{id, name}[]` for an employee's authorised sites — active
`EmployeeWorkSite` assignments, falling back to the employee's primary
location, the same rule `EmployeeWorkSiteResolver` already documents for the
device-management side. `AttendanceService.listMyWorkSites` exposes it at
`GET /attendance/correction-requests/work-sites`, gated on
`attendance.correction.create` — the same permission set that already lets
the caller create a correction, nothing new granted, and no `employeeId`
parameter exists for a caller to name someone else. `ATTENDANCE_DEVICES_READ`
was not touched, per the record's explicit instruction.

Both mount points — `corrections/new/page.tsx` and the record-page panel
(`attendance/[entryId]/page.tsx` → `AttendanceCorrectionPanel`) — now fetch
this endpoint server-side and pass the result as the form's existing
`workSites` prop, wrapped so a failure falls back to `[]` rather than
breaking the page (a correction could always be filed without naming a site).

**The manager's diff**, delivered in the same pass as the record asked:
`mapCorrectionRequest` now resolves `requestedWorkSiteId` against `Location`
(tenant-scoped) and returns `requestedWorkSiteName`. The corrections detail
page's `ChangeList` uses it for the "to" side of a work-site change,
alongside the entry's own `officeLocation.name` it already used for the
"from" side — so both sides of the diff show a name where BUG-2508's own
inline comment previously documented that only one could.

Regression coverage:
`services/api/src/modules/attendance/attendance-correction-work-sites.spec.ts`
(the endpoint's scoping and permission gate, plus the diff name resolution).
No jsdom component test was added for the two page components — this
repository's `apps/web` jest config is scoped to pure `*.spec.ts` logic (no
testing-library installed); the frontend wiring is covered by
`npm --workspace web run check-types`.

## QA Retest

Not run as a live QA session — verified by targeted regression specs and
`check-types` only. Pending a QA pass against a running tenant.

## History

- 2026-08-30 - created from qa run at `ade1fea7`.
- 2026-09-11 - fixed per EXECPLAN-0032: self-service work-sites endpoint, both
  form mount points wired, and the manager's diff resolves the requested
  site's name.
