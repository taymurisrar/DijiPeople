...
Once the direction is decided: either a UI test asserting the selector is
gone, or an integration test asserting the selected style reaches the
rendered document. No `REG-nnn` entry yet.

## Dependencies

A product decision on which resolution direction to take (see Proposed
Resolution).
