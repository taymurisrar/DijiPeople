...
Retested by unit and integration guards, not in production:

- `npx jest src/modules/billing` — 22 suites, 179 tests passing.
- `npm --workspace api run check-types` — clean.
- The deployed landing bundle and route handler were read directly and confirmed
  to send and forward the field, which is what establishes that an API-side fix
  is sufficient rather than needing a matching client change.

**Not yet retested against production.** A browser pass driving a real checkout
and reading the Customers list is the outstanding evidence, and should be run
once this is deployed. QA-COMMERCIAL-001 is that scenario, and it is NOT_RUN.
