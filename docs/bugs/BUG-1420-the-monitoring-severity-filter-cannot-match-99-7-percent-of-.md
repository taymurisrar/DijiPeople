...
Retested against production `e0aeabcd` on 2026-08-28.

The fix this record delivered is present and correct: `incidentViewWhere()`
matches `severity in ['ERROR','FATAL','error','fatal']`, so the incidents
**view** folds case as intended.

The Monitoring **overview metric** does not use it. `metrics.critical` is
still counted as `{ severity: 'ERROR' }` — exact, case-sensitive, excluding
`FATAL` — at `platform-monitoring.service.ts:103`. Filtering the queue
to Critical shows **25** incidents while the Critical tile reads **11**; the
14-row difference is rows stored as lowercase `error`, which the view
matches and the metric does not.

So the counts still disagree, on a different code path from the one this
record fixed. Recorded as [[BUG-1750]] rather than reopening, together with a
second defect on the same tile: it links to `?severity=CRITICAL`, which
matches nothing and renders "Showing 0 of 0".
