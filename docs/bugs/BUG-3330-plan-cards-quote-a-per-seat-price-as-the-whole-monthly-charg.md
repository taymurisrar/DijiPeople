...
Fixed in both halves, by two streams that each believed the other was
still missing. Neither Resolution was wrong when it was written.
...
The screen mirrors the seat arithmetic in the browser because no quote
endpoint existed when it was written. One exists now, so the
TODO(BUG-3330) in seat-pricing.ts is live rather than aspirational: the
order summary should call the seat-quote route instead of recomputing the
rule. Carried by ITEM-0175.

Both copies are pinned to the same fixture values as the server's own
seat-pricing spec, so they cannot drift silently in the meantime. That
matters here more than usual: this is the rule that already had to be
de-duplicated once on the server after two copies disagreed.
...
- Referenced by — [[ITEM-0161]], [[ITEM-0175]]
