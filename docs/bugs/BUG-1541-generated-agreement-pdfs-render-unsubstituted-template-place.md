...
**Contracts > New > From record**, source type `Customer`, record
"QA E2E Customer 20260826", template "DijiPeople Tenant Provisioning & Service
Order", **Create from record**. Refused with:

> This template needs details a customer does not carry (tenant), so the
> document would be generated with those fields blank. Create it from a source
> that has them — a provisioned tenant or an onboarding — or choose a template
> written for this source.

Rendered as `ERROR VALIDATION_FAILED` with reference
`admin_a4961f59-9569-4b46-8a77-f1ed99c11595`, expandable technical details
and a download-log affordance. Contract count was 8 before and 8 after, so the
refusal creates nothing.

**Not covered:** the success half — the same template from a *tenant* source,
which should succeed. Skipped deliberately, because
`MODULE_CAPABILITIES.contracts` is `delete: false`, so a successful create
would leave an undeletable contract in production. It needs a non-production
environment.

## History

- 2026-08-26 — found during the production admin E2E pass; recorded from the
- 2026-08-28 — refusal path verified in a browser; success path deferred to a non-production environment.
