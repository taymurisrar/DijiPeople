...
| `gateway` | `npm run gateway:build` → `gateway:test` on **windows-latest** — the .NET Integration Gateway, 104 xunit tests (ITEM-0125) | 15m | ✅ **added 2026-09-11** |
...
| `ci-required` | Aggregates the **fifteen** jobs above | 10m | ✅ **the one to require** |
