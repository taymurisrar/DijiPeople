...
export default async function TimePayrollPoliciesPage() {
  ...
  const policies = await apiRequestJson<TimePayrollPolicyRecord[]>(
    ...
    <main className="grid gap-6">
      <section className="rounded-[28px] border border-border bg-surface p-8 shadow-sm">
        ...
        <p className="mt-3 max-w-3xl text-muted">
          Control whether attendance, timesheets, no-show, and overtime feed
          payroll.
        </p>
      </section>
          ...
          policies.map((policy) => (
            ...
            >
              <div className="flex flex-wrap justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-muted">
                    ...
                    {policy.prorationBasis.replaceAll("_", " ")}
                  </p>
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted">
                <span className="rounded-xl border border-border px-3 py-1">
                  No-show: {policy.detectNoShow ? "detect" : "off"} /{" "}
                  {policy.deductNoShow ? "deduct" : "no deduction"}
                </span>
                <span className="rounded-xl border border-border px-3 py-1">
                  Overtime: {policy.overtimeEnabled ? "enabled" : "off"}
                </span>
              </div>
            </article>
          ))
