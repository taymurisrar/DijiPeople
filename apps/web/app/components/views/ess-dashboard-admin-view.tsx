...
export function EssDashboardAdminView({
  state,
  props,
}: {
    ...
    <main className="grid gap-5 lg:gap-6">
      ...
      <section className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <article className="rounded-[24px] border border-border bg-surface p-6 shadow-sm">
          ...
          <DashboardNotificationList notifications={notifications} />
        </article>
      </section>
        ...
        <StatusCard
          ...
          detail={
            productivitySummary?.lastSeenAt
              ? `Last seen ${formatDashboardDateTime(
                  productivitySummary.lastSeenAt,
                )}`
