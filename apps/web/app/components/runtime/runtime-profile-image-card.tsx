...
export type RuntimeProfileImageSummary = {
  readonly id: string;
...
type RuntimeProfileImageCardProps = {
  readonly recordId: string;
  readonly displayName: string;
  readonly profileImage: RuntimeProfileImageSummary | null;
...
export function RuntimeProfileImageCard({
  recordId,
  displayName,
  profileImage,
  resourcePath,
  ...
  const recordApiBase = `/api/${normalizedResourcePath}/${encodedRecordId}`;
  const imageUrl = `${recordApiBase}/profile-image`;

  const [firstName, lastName] = useMemo(() => {
    const parts = displayName.split(" ").filter(Boolean);
    return [parts[0] ?? displayName, parts.slice(1).join(" ")];
  }, [displayName]);
  ...
  async function handleSaveCroppedImage() {
    ...
    try {
      ...
      const response = await fetch(`${recordApiBase}/profile-image/upload`, {
        method: "POST",
        body: formData,
      });
  ...
  async function handleRemoveImage() {
    ...
    try {
      const response = await fetch(
        `${recordApiBase}/documents/${encodeURIComponent(profileImage.id)}`,
        { method: "DELETE" },
      );
      const data = (await response.json().catch(() => null)) as
        | { message?: string }
      ...
      <article className="rounded-[24px] border border-border bg-surface p-6 shadow-sm">
        ...
        <div className="mt-5 flex min-w-0 gap-4">
          <UserAvatar
            ...
            firstName={firstName}
            imageSrc={profileImage ? imageUrl : null}
            lastName={lastName}
            size="lg"
          />
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-muted">
                  ...
                  avatar matches this preview exactly.
                </p>
              </div>
              <button
