  ...
  return (
    <section className="grid gap-4 rounded-lg border border-border bg-surface p-4">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <AgentMetric
          label="Live status"
          value={stringValue(data.liveStatus) || "Offline"}
        />
        <AgentMetric
          label="Active time"
      ...
      <DataTable
        columns={[
          ...
          },
          {
            key: "status",
            header: "Status",
            render: (row) => stringValue(row.status) || "Unknown",
          },
          {
            key: "agentVersion",
          ...
          },
          {
            ...
            render: (row) =>
              formatDateTime(
                stringValue(row.lastSeenAt) || stringValue(row.lastHeartbeatAt),
                runtime?.tenant,
              ) || "Not set",
          },
        ]}
...
function EmployeeProfilePhotoWidget({
  ...
  const canRemove =
    permissionKeys.has("employees.documents.delete") ||
    permissionKeys.has("employees.update.self");
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-3">
        ...
        <div className="flex flex-wrap gap-2">
          <label className="relative">
            ...
            <input
              className="h-9 rounded-md border border-border pl-9 pr-3 text-sm"
