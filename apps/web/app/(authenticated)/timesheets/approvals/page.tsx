            ...
            <div className="mt-5 grid gap-3">
              {response.items.map((timesheet) => (
                <Link
                  ...
                  href={`/timesheets/approvals?${queueHref(params, timesheet.id)}`}
                >
  ...
  return (
    <div className="flex items-center gap-3">
      <Link
        ...
        href={`/timesheets/approvals?year=${prev.getFullYear()}&month=${prev.getMonth() + 1}&status=${status}`}
      >
        Previous
      </Link>
      ...
      <Link
        className="rounded-2xl border border-border px-4 py-3 text-sm font-medium text-foreground transition hover:border-accent/30 hover:text-accent"
        href={`/timesheets/approvals?year=${next.getFullYear()}&month=${next.getMonth() + 1}&status=${status}`}
      >
        Next
      </Link>
    </div>
