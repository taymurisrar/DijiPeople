...
describe("BUG-1964 — a dialog that creates one record is titled in the singular", () => {
  ...
  /*
   * `buildSubgridQuickCreate` moved to `lib/runtime/quick-create-metadata.ts`
   * when the runtime stream extracted it so its behaviour could be tested at
   * all. The generic builder stayed in the component. So this guard reads two
   * files now — and it has to keep reading both, because the defect was that
   * the two title paths disagreed, and a guard over only one of them would go
   * green while the other regressed.
   */
  const quickCreate = source("lib/runtime/quick-create-metadata.ts");
  ...
  it("singularises the related-list quick-create titles", () => {
    ...
    expect(quickCreate).not.toContain("displayName: `New ${subgrid.title}`");
    expect(quickCreate).toContain(
