...
type LocationBridge = {
  getLocationRequest?: () => Promise<AgentLocationRequest | null>;
  captureDesktopLocation?: () => Promise<DesktopLocationResult>;
  submitLocationResult?: (
    result: AgentLocationResult,
  ) => Promise<{ ok: true } | { ok: false; message: string }>;
};

const locationWindow = window as Window & { dijiAgent?: LocationBridge };
...
const GEOLOCATION_ATTEMPT_TIMEOUT_MS = 20_000;
const LOCATION_RETRY_DELAY_MS = 5_000;

let currentRequest: AgentLocationRequest | null = null;
...
async function initialize() {
  ...
  summary.textContent = `Requested ${formatDateTime(
    currentRequest.requestedAt,
  )}. This request expires ${formatDateTime(currentRequest.expiresAt)}.`;
...
async function capturePreciseLocationUntilExpiry(): Promise<
  | {
      ok: true;
      latitude: number;
      longitude: number;
  ...
  | { ok: false; status: "DENIED" | "FAILED"; message: string }
> {
  ...
  return {
    ok: false,
    status: "FAILED",
    message: compactLocationError(
      `${lastError} The request expired before a precise location was captured.`,
    ) ?? "The request expired before a precise location was captured.",
  };
}
...
async function captureLocationFallback(
  primaryError: string,
): Promise<DesktopLocationResult> {
  setMessage(
    "Device location failed. Trying Windows Location Services...",
    "info",
  );
  ...
  return "message" in fallback
    ? {
        ok: false,
        message:
          compactLocationError(`${primaryError} ${fallback.message}`) ??
...
async function submitResult(
  result: Omit<AgentLocationResult, "requestId" | "deviceId">,
) {
  if (!currentRequest) return;
  ...
  const response =
    await locationWindow.dijiAgent?.submitLocationResult?.({
      ...result,
  ...
  if (response?.ok) {
    setMessage(
      result.status === "CAPTURED" ? "Location shared." : "Response sent.",
      "success",
    );
    return;
  }
...
async function handlePermissionState() {
  ...
  if (permission === "denied") {
    setMessage(
      "Location is blocked on this device. Enable location permission, then try again.",
      "error",
    );
    return;
  }
...
function readLocationErrorStatus(error: unknown): "DENIED" | "FAILED" {
...
function readLocationErrorMessage(error: unknown) {
...
function compactLocationError(value?: string) {
...
function setMessage(text: string, tone: "info" | "success" | "error") {
