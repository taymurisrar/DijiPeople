 ...
 */
async function captureLocationUntilAccurate(): Promise<
  | {
      ok: true;
      latitude: number;
      longitude: number;
  ...
  for (let attempt = 1; attempt <= MAX_CAPTURE_ATTEMPTS; attempt += 1) {
    if (isLocationRequestExpired()) {
      return {
        ...
        status: "FAILED",
        message: compact(
          `${lastError} The request expired before a usable location was captured.`,
        ),
      };
    }
...
function compact(value: string) {
  ...
  if (!normalized) return "Unable to capture location.";
