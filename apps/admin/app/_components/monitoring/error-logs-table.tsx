    ...
    <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
      <div className="grid gap-3 border-b border-slate-200 bg-slate-50 p-4 md:grid-cols-5">
        <input
          ...
          onChange={(event) =>
            updateFilter(() => setReference(event.target.value))
          }
        ...
        <FilterSelect
          ...
          values={unique(logs.map((log) => log.severity))}
        />
        <FilterSelect
          label="All source apps"
          onChange={(value) => updateFilter(() => setSource(value))}
          values={unique(logs.map((log) => log.sourceApp))}
        />
        <FilterSelect
          label="All tenant IDs"
          onChange={(value) => updateFilter(() => setTenantId(value))}
          values={unique(logs.map(getTenantId))}
        />
        <FilterSelect
          label="All environments"
          onChange={(value) => updateFilter(() => setEnvironment(value))}
          values={unique(logs.map((log) => log.environment))}
        />
      </div>
      <DataTable
        rows={visibleRows}
