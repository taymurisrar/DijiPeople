...
/*
 * Task records already NAME the bugs and backlog items they cover — in prose,
 * in plain text, where Obsidian cannot see them. TASK-0006 mentions six BUG ids
 * and three ITEM ids and was still an isolated dot in the graph.
 *
 * This projects relationships that already exist; it invents none. AFFECTED_MODULES
 * links only on an EXACT match between a path's last segment and a module note
 * — `services/api/src/modules/billing` -> [[billing]], while `apps/landing` links
 * to nothing because no `landing` module note exists. A plausible-looking wrong
 * edge is worse than an absent one.
 */
const TASK_GRAPH_BEGIN =
  '<!-- GRAPH:BEGIN — generated by scripts/rebuild-tasks.mjs; edit the record, not this block -->';
const TASK_GRAPH_END = '<!-- GRAPH:END -->';

const moduleNotes = new Set(
...
for (const task of tasks) {
  const current = readFileSync(task.path, 'utf8');
  const normalised = current.replace(/\r\n/g, '\n');
  const outsideBlock = normalised.split(TASK_GRAPH_BEGIN)[0];

  const modules = (task.modules ?? [])
    .map((entry) => String(entry).trim().split('/').pop())
    .filter((name) => moduleNotes.has(name));
  const records = [
    ...new Set((outsideBlock.match(/\b(?:BUG|ITEM)-\d{4}\b/g) ?? [])),
  ].sort();

  const rows = [];
  if (records.length) rows.push(`- Records — ${records.map((id) => `[[${id}]]`).join(', ')}`);
  if (modules.length) {
    rows.push(`- Modules — ${[...new Set(modules)].map((name) => `[[${name}]]`).join(', ')}`);
  }
  if (!rows.length) {
    rows.push(
  ...
  const block = [TASK_GRAPH_BEGIN, '', '## Related', '', ...rows, '', TASK_GRAPH_END].join('\n');
  const start = normalised.indexOf(TASK_GRAPH_BEGIN);
  const next =
    start === -1
      ? `${normalised.replace(/\s*$/, '')}\n\n${block}\n`
      : `${normalised.slice(0, start)}${block}${normalised.slice(
          normalised.indexOf(TASK_GRAPH_END, start) + TASK_GRAPH_END.length,
        )}`;

  if (next === normalised) continue;
  if (checkOnly) {
    stale.push(task.relative);
    continue;
  }
  writeFileSync(task.path, current.includes('\r\n') ? next.replace(/\n/g, '\r\n') : next);
  written += 1;
}
